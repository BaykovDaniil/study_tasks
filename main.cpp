...
void py() {
	...
	fprintf(res, "plt.figure(figsize = (16, 9))\n");
...
int main()
{
	double a = 1, b = 3; // начало и конец отрезка

	const int K = 5; // количество конечных элементов
	const int N = 4; // количество узлов на 1 конечном элементе
	const int L = 1000; // количество случайных точек
	...
	py();
	std::system("python3 plot.py"); // эта команда вызывает командную строку и включает питоновскую часть задачи
